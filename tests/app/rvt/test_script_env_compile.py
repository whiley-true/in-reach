"""Env profiles through a real Apply: ``${CONSTANTS}`` and ``-- @if`` blocks in ``script/output.txt``.

The unit tests for the preprocessor itself are ``tests/app/test_script_preprocess.py``; these prove the
compiler is actually handed the *processed* text, that switching profiles changes what's built, and that
...
def _project(tmp_path: Path, script: str = _SCRIPT) -> tuple[Path, Path]:
    ...
    # A new project already ships starter dev/release profiles with dev active (see new_project); these
    ...
    script_preprocess.set_active_profile(folder, None)
...
def test_an_undefined_constant_fails_the_build_at_its_line_and_column(tmp_path: Path) -> None:
    ...
    assert "error (2:5): '${NOPE}' isn't defined in the 'dev' profile" in compile_module.format_build_result(result)
...
def test_a_constant_with_no_profile_active_says_so(tmp_path: Path) -> None:
    ...
    assert "no environment profile is active" in compile_module.format_build_result(result)
...
def test_a_broken_env_file_fails_the_build_naming_that_file(tmp_path: Path) -> None:
    ...
    assert result.success is False
    assert result.failure is not None and result.failure.startswith("dev.env, line 2:")
...
def test_an_active_profile_whose_file_was_deleted_is_a_clear_failure(tmp_path: Path) -> None:
    ...
    assert result.success is False
    assert "the active profile 'dev' has no file" in (result.failure or "")
