...
def test_decompile_into_project_writes_schema_files_under_settings_only(
    tmp_path: Path, monkeypatch
) -> None:
    # PROMPT.md: "in settings, please not[e] examples from repo v2 ... where we have a schema,
    # please re-add this to the top of the files" -- settings/ gets a schema/ subfolder; build/
    # (disposable, autogenerated output) doesn't need one.
    settings = _game_settings()
    variant = _FakeVariant(_FakeMultiplayer())
    _patch(monkeypatch, variant, settings)
    ...
    schema_dir = folder / "settings" / "schema"
    ...
    assert not (folder / "build" / "schema").exists()
...
def test_resync_from_bin_also_regenerates_the_schema_files(tmp_path: Path, monkeypatch) -> None:
    ...
    assert (folder / "settings" / "schema" / decompile.SETTINGS_SCHEMA_FILENAME).is_file()
