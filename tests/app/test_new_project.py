...
def test_set_project_title_rewrites_just_the_heading_line(tmp_path: Path) -> None:
    folder = tmp_path / "abcd1234"
    folder.mkdir()
    (folder / "README.md").write_text("# Old Title\n\nSome description.\n\n## Layout\n", encoding="utf-8")

    new_project.set_project_title(folder, "New Title")

    readme = (folder / "README.md").read_text(encoding="utf-8")
    assert readme == "# New Title\n\nSome description.\n\n## Layout\n"


def test_set_project_title_is_a_no_op_when_already_matching(tmp_path: Path) -> None:
    folder = tmp_path / "abcd1234"
    folder.mkdir()
    readme_path = folder / "README.md"
    readme_path.write_text("# Same Title\n\nDescription.\n", encoding="utf-8")
    original_mtime = readme_path.stat().st_mtime_ns

    new_project.set_project_title(folder, "Same Title")

    assert readme_path.stat().st_mtime_ns == original_mtime


def test_set_project_title_is_a_no_op_with_no_readme(tmp_path: Path) -> None:
    folder = tmp_path / "abcd1234"
    folder.mkdir()

    new_project.set_project_title(folder, "New Title")  # should not raise

    assert not (folder / "README.md").is_file()


def test_set_project_title_then_read_project_title_round_trips(tmp_path: Path) -> None:
    folder = tmp_path / "abcd1234"
    folder.mkdir()
    (folder / "README.md").write_text("# Old Title\n", encoding="utf-8")
    ...
    assert new_project.read_project_title(folder) == "New Title"
