...
from in_reach.app import env_file, new_project, script_preprocess
...
def test_create_gametype_project_names_the_folder_with_a_generated_id_not_the_title(
    ...
    # own repo-level .gitignore -- except for the handful of generated files PROMPT.md asked to
    # keep trackable, so a diff of those stays visible in review.
    gitignore = (folder / "build" / ".gitignore").read_text(encoding="utf-8")
    assert gitignore == (
        "*\n"
        "!.gitignore\n"
        "!Compiled.txt\n"
...
def test_create_gametype_project_decompiles_a_real_source_variant(
    ...
    script = (folder / "script" / "output.mgl").read_text(encoding="utf-8")
...
def test_create_gametype_project_surfaces_a_decompile_failure_as_a_warning_not_a_crash(
    ...
    assert not (folder / "script" / "output.mgl").is_file()
...
def _engine_metadata(folder: Path, directory: str = "settings", name: str = "settings.json") -> tuple:
    metadata = json.loads((folder / directory / name).read_text(encoding="utf-8"))["multiplayer"]["game_settings"]["metadata"]
    return metadata["engine_category"], metadata["categorization_icon"]


@_NEEDS_NATIVE_RVT_AND_JUGGERNAUT_BIN
def test_the_chosen_category_is_also_the_engines_so_a_build_applies_it(project_dir: Path) -> None:
    """The category picked when creating a project goes into multiplayer.game_settings.metadata -- what a build writes
    into the variant -- not only meta.category, the project's own label. The build snapshot still says what the source
    .bin holds, so the difference is a change the first build applies."""
    folder, _warning = new_project.create_gametype_project(
        project_dir, "Slayer Plus", category=EngineCategory.slayer, source_variant=_JUGGERNAUT_BIN
    )

    assert _engine_metadata(folder) == (int(EngineCategory.slayer), int(EngineIcon.slayer))  # stored as numbers
    assert _engine_metadata(folder, "build", "settings.autogenerated.json") != (1, 1)  # the source .bin's own (juggernaut)


@_NEEDS_NATIVE_RVT_AND_JUGGERNAUT_BIN
def test_no_category_leaves_the_source_variants_own_category_alone(project_dir: Path) -> None:
    folder, _warning = new_project.create_gametype_project(project_dir, "Plain", source_variant=_JUGGERNAUT_BIN)

    assert _engine_metadata(folder) == _engine_metadata(folder, "build", "settings.autogenerated.json")
...
def test_create_gametype_project_with_no_source_variant_writes_no_settings_json(
    ...
    # had no script/output.mgl content in that case.
    folder, warning = new_project.create_gametype_project(
...
def test_a_new_project_starts_with_one_env_base(project_dir: Path) -> None:
    folder, _ = new_project.create_gametype_project(project_dir, "Envs")
    assert script_preprocess.list_envs(folder) == ["base"]  # a release env is added when it's wanted


def test_a_new_project_builds_with_base(project_dir: Path) -> None:
    folder, _ = new_project.create_gametype_project(project_dir, "Envs")
    assert script_preprocess.active_env_name(folder) == "base" == new_project.DEFAULT_ENV


def test_the_starter_env_is_valid_and_only_sets_dev(project_dir: Path) -> None:
    folder, _ = new_project.create_gametype_project(project_dir, "Envs")
    base = script_preprocess.load_env(folder, "base")
    assert base.flags == {"DEV"}
    assert base.constants == {}  # the example constant is only commented out


def test_the_starter_env_explains_itself(project_dir: Path) -> None:
    folder, _ = new_project.create_gametype_project(project_dir, "Envs")
    text = (folder / "script" / "env" / "base.env").read_text(encoding="utf-8")
    assert text.startswith('# Env "base"')
...
def test_a_dev_block_is_on_in_a_new_project(project_dir: Path) -> None:
    """base (FLAGS=DEV) is the default: `-- @if DEV` lines are built until another env is chosen."""
    folder, _ = new_project.create_gametype_project(project_dir, "Envs")
    assert "debug()" in script_preprocess.preprocess_project(folder, "-- @if DEV\ndebug()\n-- @end\n")
    script_preprocess.create_env(folder, "release")
...
def test_the_starter_profile_files_are_part_of_the_initial_snapshot(project_dir: Path) -> None:
    ...
    assert vcs.env_at(folder, vcs.history(folder)[0].sha) == "base"  # the env in use is part of it too
...
# -- script/output.txt -> script/output.mgl -----------------------------------------------------------------


def test_the_script_is_output_mgl(project_dir: Path) -> None:
    assert new_project.SCRIPT_FILENAME == "output.mgl" and new_project.LEGACY_SCRIPT_FILENAME == "output.txt"


def test_an_old_projects_output_txt_is_renamed_the_first_time_the_script_is_read(tmp_path: Path) -> None:
    from in_reach import api

    (tmp_path / "script").mkdir()
    (tmp_path / "script" / "output.txt").write_text("game.end_round()\n", encoding="utf-8")

    result = api.check(tmp_path)

    assert result.ok
    assert not (tmp_path / "script" / "output.txt").exists()
    assert (tmp_path / "script" / "output.mgl").read_text(encoding="utf-8") == "game.end_round()\n"


def test_an_existing_output_mgl_is_never_overwritten_by_an_old_output_txt(tmp_path: Path) -> None:
    (tmp_path / "script").mkdir()
    (tmp_path / "script" / "output.txt").write_text("old\n", encoding="utf-8")
    (tmp_path / "script" / "output.mgl").write_text("new\n", encoding="utf-8")

    path = new_project.migrate_script_file(tmp_path)

    assert path.read_text(encoding="utf-8") == "new\n" and (tmp_path / "script" / "output.txt").exists()


# -- personal files -----------------------------------------------------------------------------------------


def test_a_new_project_gitignores_its_notepad(tmp_path: Path) -> None:
    folder = tmp_path / "abcd1234"
    folder.mkdir()

    new_project.ensure_gitignore(folder)

    lines = (folder / ".gitignore").read_text(encoding="utf-8").splitlines()
    assert "/Notes.txt" in lines and lines[0].startswith("#")


def test_ensure_gitignore_adds_to_an_existing_one_once(tmp_path: Path) -> None:
    (tmp_path / ".gitignore").write_text("*.log", encoding="utf-8")

    new_project.ensure_gitignore(tmp_path)
    new_project.ensure_gitignore(tmp_path)

    assert (tmp_path / ".gitignore").read_text(encoding="utf-8") == "*.log\n/Notes.txt\n"


def test_ensure_gitignore_leaves_one_that_already_lists_it(tmp_path: Path) -> None:
    (tmp_path / ".gitignore").write_text("Notes.txt\n", encoding="utf-8")

    new_project.ensure_gitignore(tmp_path)

    assert (tmp_path / ".gitignore").read_text(encoding="utf-8") == "Notes.txt\n"


def test_only_the_top_level_notepad_is_personal() -> None:
    assert new_project.is_personal_file(Path("Notes.txt"))
    assert not new_project.is_personal_file(Path("script") / "Notes.txt")
