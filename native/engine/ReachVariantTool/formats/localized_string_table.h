...
class ReachString : public indexed_list_item {
   ...
   public:
      ...
      inline bool has_content(reach::language l) const noexcept {
         // (offsets[l] == -1) means this language was never saved -- get_content() still
         // returns a valid (empty, default-constructed) std::string in that case, so this is
         // the only way to tell "language absent" apart from "language present but empty."
         return this->offsets[(int)l] != -1;
      }
      //
