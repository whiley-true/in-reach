...
def is_generated_file(path: Path) -> bool:
    ...
    ``.bin``/``.mglo`` and its ``*.autogenerated.json`` re-extraction snapshots (PROMPT.md) -- never
    meant to be hand-edited, since the next successful build/resync overwrites it wholesale. The
    IDE opens one read-only rather than letting an edit silently vanish on the next one (see
    :meth:`in_reach.ide.tabs.TabPane.open_file`).
...
def _decompile_source_variant(
    ...
    *,
    ...
    project's own category/category_icon into the resulting ``settings.json`` (PROMPT.md: "move
    category and category_icon into settings.json") and ``valid_maps.json`` narrowed down from
    ``map_entries`` (PROMPT.md: "inside settings/maps we should [have] valid_maps.json instead ...
    this should only fill with maps that are valid for this gametype") -- imported lazily so that
    creating a blank project (no ``source_variant``) never pays for importing pydantic/the native
    RVT extension at all.
    ...
    *,
    ...
    if source_variant is not None:
        ...
        decompile_warning = _decompile_source_variant(
            copied_variant, folder, category=category, category_icon=resolved_icon, map_entries=map_entries
        )
