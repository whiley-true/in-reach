...
- `{settings}/` -- a live mirror of the source `.bin`'s own settings (edited through RVT's own GUI,
  not by hand here).
- `{schemas}/` -- the JSON Schema files `{settings}/`'s own JSON validates against, regenerated
  fresh on every decompile/resync -- never hand-edited either.
- `{build}/` -- the same data as `{settings}/` again, as a disposable, point-in-time snapshot
  (`*.autogenerated.json`), plus `valid_maps.json` -- the maps this gametype can actually be played
  on, narrowed down from `.in-reach/{maps_json}`'s full scan -- and the compiled `.bin` under
  `{build}/{dist}/` once compiling exists. Always gitignored -- it can be regenerated from
...
def set_project_title(folder: Path, title: str) -> None:
    """Rewrites ``folder``'s README so its own title heading (what :func:`read_project_title` reads
    back) matches ``title``, leaving the rest of the README (description, Layout section) alone.

    Called whenever a project's title changes after creation -- via RVT itself renaming the
    gametype (its embedded ``.bin`` header title now flows straight through
    :func:`~in_reach.app.rvt.decompile.resync_from_bin`, see that function's own docstring) or via a
    hand-edited ``settings/settings.json`` ``meta.title`` an Apply just compiled (PROMPT.md: "when a
    project name is changed [via rvt or via apply settings.json change] - the project title should
    change in the tabs and in the breadcrumb") -- so every reader of :func:`read_project_title` (the
    Explorer's own project tabs, the Welcome tab's Recent list, the File menu's Open Recent, and an
    open editor's breadcrumb) stays in sync without needing to know about ``settings.json`` at all.

    A no-op if ``folder`` has no README yet, or its title heading already matches ``title``.

    Args:
        folder: A gametype project folder.
        title: The new title to stamp into the README's own heading line.
    """
    readme = folder / README_FILENAME
    ...
    for i, line in enumerate(lines):
        if line.startswith(_README_TITLE_LINE_PREFIX):
            ending = "\n" if line.endswith("\n") else ""
            current = line[len(_README_TITLE_LINE_PREFIX):].rstrip("\n")
            if current == title:
                return
            lines[i] = f"{_README_TITLE_LINE_PREFIX}{title}{ending}"
