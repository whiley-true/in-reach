...
def count_script_strings(path: Path) -> int | None:
    """How many entries ``path`` (a ``strings.json``) own ``script_strings`` table has -- backs
    the Dashboard's own Stats box (PROMPT.md: "please also make stats include[] strings.json"),
    shown there regardless of whether ``build/stats.autogenerated.json`` exists at all (unlike that
    file, ``strings.json`` is written for every project, multiplayer or not -- see
    :mod:`in_reach.app.rvt.decompile`'s own ``_write_generated_files``).

    Returns ``None`` if ``path`` doesn't exist or fails to parse/validate -- same "never raise, just
    nothing to show" reasoning as :func:`~in_reach.app.rvt.settings_io.load_build_stats`.
    """
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    ...
    data.pop("$schema", None)
    try:
        document = StringsDocument.model_validate(data)
    except ValidationError:
        return None
    return len(document.script_strings)
