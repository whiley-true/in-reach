...
disposable ``*.autogenerated.json`` snapshot of the same data), and seeds ``script/output.txt`` --
...
``script/output.txt``, ``settings/`` and the matching ``build/*.autogenerated.json`` snapshot from
...
SCRIPT_FILENAME = "output.txt"  # PROMPT.md: "please rename game.txt to output.txt"
...
def write_build_snapshot(
    ...
    *,
    ...
    # The script this .bin holds, and which output.txt it was built from -- what lets a later RVT save
...
def _resync_from_bin_in_process(
    ...
    *,
    ...
    rvt saves". ``script/output.txt`` is left completely alone: this is meant to be called
    ...
    ``output.txt``; the IDE's file watcher does (``MainWindow._pull_script_from_rvt``).
    ...
    # Records what the .bin's script now says, but leaves output.txt alone: whether RVT's version should
