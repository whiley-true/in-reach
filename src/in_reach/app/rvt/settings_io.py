...
def load_build_stats(path: Path) -> BuildStats | None:
    """Reads ``path`` (normally ``build/stats.autogenerated.json``) back into a :class:`BuildStats`
    -- backs the Dashboard's own read-only Stats box (PROMPT.md: "please also add a stats view
    into the dashboard").

    Returns ``None`` if ``path`` doesn't exist or fails to parse/validate -- same "never raise, just
    fall back to nothing to show" reasoning as :func:`load_script_settings`; a Firefight/blank
    project that's never produced a multiplayer build never has this file at all (see
    :mod:`in_reach.app.rvt.decompile`'s own module docstring), which is a normal, common case here,
    not an error.
    """
    try:
