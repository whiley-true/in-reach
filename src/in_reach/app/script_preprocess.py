...
and one of them is *active* (``script/env/active_env.txt`` holds its name). Before the script is
...
- ``-- @if DEV`` ... ``-- @end`` keeps the block only when ``DEV`` is one of the env's flags
...
- **No env and no directives means unchanged.** ``preprocess(text)`` returns ``text`` exactly, so
...
ACTIVE_ENV_FILENAME = new_project.ACTIVE_ENV_FILENAME
FLAGS_KEY = "FLAGS"
...
class PreprocessError(ValueError):
    """Something in the script or the active env that stops it being preprocessed.
...
def parse_env(name: str, text: str, path: Path | None = None) -> Env:
    ...
    for number, raw in enumerate(text.splitlines(), start=1):
        ...
        constants[key] = value
    return Env(name, flags, constants)


def env_dir(folder: Path) -> Path:
    """``<project>/script/env`` -- where envs live (need not exist)."""
    return folder / new_project.SCRIPT_DIRNAME / ENV_DIRNAME


def list_envs(folder: Path) -> list[str]:
    """The names of ``folder``'s envs (``dev`` for ``script/env/dev.env``), sorted."""
...
def active_env_name(folder: Path) -> str | None:
    """The env ``folder`` is set to build with, or ``None`` if none is chosen. A project from before envs were called
    envs names it in ``active_profile.txt``, which is read when there is no ``active_env.txt``."""
    for filename in (ACTIVE_ENV_FILENAME, new_project.LEGACY_ACTIVE_ENV_FILENAME):
        try:
            name = (env_dir(folder) / filename).read_text(encoding="utf-8").strip()
        except OSError:
            continue
        return name or None
    return None
...
def set_active_env(folder: Path, name: str | None) -> None:
    """Chooses the env to build with (``None`` clears the choice). Removes an old ``active_profile.txt``.

    Raises:
        ValueError: ``name`` isn't one of :func:`list_envs`.
    """
    path = env_dir(folder) / ACTIVE_ENV_FILENAME
    if name is not None and name not in list_envs(folder):
        raise ValueError(f"{name!r} isn't an env of this project (have: {', '.join(list_envs(folder)) or 'none'})")
    (env_dir(folder) / new_project.LEGACY_ACTIVE_ENV_FILENAME).unlink(missing_ok=True)
    if name is None:
...
def env_path(folder: Path, name: str) -> Path:
    return env_dir(folder) / f"{name}{ENV_SUFFIX}"


def create_env(folder: Path, name: str, *, copy_from: str | None = None) -> Path:
    """Adds ``script/env/<name>.env``: a copy of env ``copy_from``, or an empty one (no flags, no constants) with the
    usual explanatory header. Returns its path.

    Raises:
        ValueError: ``name`` isn't a usable file name, is taken, or ``copy_from`` doesn't exist."""
    if not _ENV_NAME.fullmatch(name):
        raise ValueError(f"{name!r} can't be an env name (letters, digits, '_' and '-')")
    path = env_path(folder, name)
    if path.exists():
        raise ValueError(f"there is already an env named {name!r}")
    if copy_from is not None:
        source = env_path(folder, copy_from)
...
def delete_env(folder: Path, name: str) -> None:
    """Removes env ``name``; if it was the active one, no env is active afterwards.

    Raises:
        ValueError: There is no env ``name``."""
    path = env_path(folder, name)
    if not path.is_file():
        raise ValueError(f"there is no env named {name!r}")
    path.unlink()
    if active_env_name(folder) == name:
        set_active_env(folder, None)


def load_env(folder: Path, name: str) -> Env:
    ...
    try:
        ...
        raise PreprocessError(f"the active env {name!r} has no file {path}", 0, 0, path) from None
    return parse_env(name, text, path)
...
def preprocess(source: str, env: Env | None = None) -> str:
    """``source`` with ``env``'s ``@if`` blocks resolved and its ``${NAME}`` constants substituted.

    Args:
        source: The script text.
        env: The env to apply, or ``None`` for none (no flags, no constants -- so a ``${X}``
    ...
    """
    flags = env.flags if env else frozenset()
    stack: list[_Block] = []
    out: list[str] = []
    for number, raw in enumerate(source.splitlines(keepends=True), start=1):
        ...
        if directive:
            ...
            continue
        out.append(_substitute_line(body, number, env) + eol if active else eol)
    if stack:
...
def _substitute_line(line: str, number: int, env: Env | None) -> str:
    ...
    if annotation is None:
        return _substitute(line, number, env)
    ...
    return prefix + _substitute(" " * len(prefix) + annotation.group(2), number, env)[len(prefix):]


def _substitute(line: str, number: int, env: Env | None) -> str:
    ...
    while i < len(line):
        ...
        if line.startswith("${", i):
            ...
            out.append(_value_of(match.group(1).strip(), number, i, in_string, env))
            i = match.end()
            continue
        out.append(char)
        i += 1
...
def _value_of(name: str, number: int, col: int, in_string: bool, env: Env | None) -> str:
    if not _NAME.fullmatch(name):
        raise PreprocessError(f"'${{{name}}}' isn't a valid constant name", number, col)
    if env is None:
        raise PreprocessError(f"'${{{name}}}' has no value: no env is active", number, col)
    value = env.constants.get(name)
    if value is None:
        raise PreprocessError(f"'${{{name}}}' isn't defined in the {env.name!r} env", number, col)
...
def preprocess_project(folder: Path, source: str) -> str:
    """:func:`preprocess` ``source`` with ``folder``'s active env, if it has one.

    Raises:
        PreprocessError: The active env's file is missing or invalid, or see :func:`preprocess`.
    """
    name = active_env_name(folder)
    env = load_env(folder, name) if name else None
    return preprocess(source, env)
