""""Apply" -- compiles a project's hand-edited ``settings/*.json`` + ``script/output.mgl`` into a
...
work (loads this project's own base ``.bin``, compiles ``script/output.mgl``, applies every
...
def apply_settings_changes(project_dir: Path, folder: Path) -> BuildResult:
    """Compiles ``folder``'s current ``settings/*.json`` + ``script/output.mgl`` into a real
